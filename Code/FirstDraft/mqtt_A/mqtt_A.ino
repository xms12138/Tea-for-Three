...
// LED strip controlled by topic B
LedMode led1Mode = LED_OFF;
unsigned long led1BlinkStartMs = 0;
unsigned long led1LastToggleMs = 0;
bool led1BlinkVisible = false;

// LED strip controlled by topic C
LedMode led2Mode = LED_OFF;
unsigned long led2BlinkStartMs = 0;
unsigned long led2LastToggleMs = 0;
bool led2BlinkVisible = false;

const unsigned long BLINK_DURATION_MS = 3000;   // total blinking duration
const unsigned long BLINK_INTERVAL_MS = 250;    // blink interval

// ========= FSR State =========
bool currentStateAOn = false;   // true = pressure detected (ON)
...
// ===== Helper: set full LED strip color =====
...
// ===== Render LED1 according to current mode =====
void renderLed1() {
  if (led1Mode == LED_ON) {
...
// ===== Render LED2 according to current mode =====
void renderLed2() {
  if (led2Mode == LED_ON) {
...
// ===== Debounce logic for FSR =====
// currentStateAOn = true  -> pressure detected
// currentStateAOn = false -> no pressure
